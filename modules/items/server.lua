...
Item('taco', function(event, item, inventory, slot, data)
   if event == 'usingItem' then
	  exports.ox_inventory:AddItem(source, 'garbage', 1, {image = 'trash_paper'})
   end
end)

Item('sandwich', function(event, item, inventory, slot, data)
   if event == 'usingItem' then
	  exports.ox_inventory:AddItem(source, 'garbage', 1, {image = 'trash_paper'})
   end
end)

Item('burrito', function(event, item, inventory, slot, data)
   if event == 'usingItem' then
	  exports.ox_inventory:AddItem(source, 'garbage', 1, {image = 'trash_paper'})
   end
end)

Item('sodajari', function(event, item, inventory, slot, data)
   if event == 'usingItem' then
	  exports.ox_inventory:AddItem(source, 'garbage', 1, {image = 'trash_can'})
   end
end)
